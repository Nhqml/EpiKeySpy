...
class Event:
    ...
    def from_raw(cls, raw_event: bytes) -> Event:
        ...
        EventClass = MAP_TYPE_EVENT.get(cls.Type(type_), Event)
        return EventClass(datetime.fromtimestamp(sec) + timedelta(microseconds=usec), cls.Type(type_), code, value)
...
class EventKey(Event):
    ...
    class Code(Event.Code):
        """Key codes gathered from /usr/include/linux/input-event-codes.h
        """
        KEY_RESERVED = 0
        KEY_ESC = 1
        KEY_1 = 2
